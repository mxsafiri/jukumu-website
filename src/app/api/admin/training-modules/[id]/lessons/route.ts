...
// Add a new lesson to a training module
export async function POST(
  ...
  try {
    ...
    const moduleCheck = await client.query('SELECT id FROM training_modules WHERE id = $1', [id]);
    ...
    if (!order) {
      const orderResult = await client.query(
        'SELECT COALESCE(MAX(lesson_order), 0) + 1 as next_order FROM training_lessons WHERE training_module_id = $1',
        [id]
      );
    ...
    const result = await client.query(`
      INSERT INTO training_lessons (training_module_id, title, content, lesson_order, duration_minutes, lesson_type, video_url)
