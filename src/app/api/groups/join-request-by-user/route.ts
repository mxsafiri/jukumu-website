  ...
  try {
    ...
    if (!userId || !groupId) {
      return NextResponse.json({ 
        error: 'User ID na Group ID ni lazima.' 
      }, { status: 400 });
    }
    ...
    try {
      ...
      const memberCheck = await client.query(
        'SELECT id, full_name, email FROM members WHERE user_id = $1',
        [userId]
      );
      ...
      if (memberCheck.rows.length === 0) {
        client.release();
        return NextResponse.json({ 
          error: 'Hakuna rekodi ya mwanachama kwa mtumiaji huyu. Jisajili kwanza kama mwanachama.' 
        }, { status: 404 });
      }
      ...
      // Create join request using the correct member ID
      ...
      if (groupLeaderId) {
        try {
          ...
          `, [
            groupLeaderId, // user_id (group leader's user ID)
